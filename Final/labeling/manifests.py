...
def build_local_sprint3_manifest(sprint3_output_dir: str | Path, site_id: str, variant: str) -> pd.DataFrame:
    sprint3_output_dir = Path(sprint3_output_dir)
    shrubs = sorted((sprint3_output_dir / 'Shrubs').glob('*.csv'))
    fuels = sorted((sprint3_output_dir / 'Fuels').glob('*.tif'))
    inventory = sorted((sprint3_output_dir / 'Inventory').glob('*.csv'))
    metrics = sorted((sprint3_output_dir / 'metrics').glob('*.csv'))
    return pd.DataFrame([{
        'site_id': site_id,
        'variant': variant,
        'sprint3_output_dir': str(sprint3_output_dir),
        'n_shrub_csv': len(shrubs),
        'n_fuels_tif': len(fuels),
        'n_inventory_csv': len(inventory),
        'n_metrics_csv': len(metrics),
        'shrub_csv_paths': [str(p) for p in shrubs],
        'fuels_tif_paths': [str(p) for p in fuels],
        'inventory_csv_paths': [str(p) for p in inventory],
        'metrics_csv_paths': [str(p) for p in metrics],
    }])
